...
void ALocalizacaoHUD::DrawHUD()
{
    ...
    // ── Fonte de dados: carrinho GPS ou actor legado ──────────────────────────
    const AMyGolfCartPawn* GolfCart = Cast<AMyGolfCartPawn>(
        UGameplayStatics::GetPlayerPawn(this, 0));
    const AMyActor* Actor = Cast<AMyActor>(
        UGameplayStatics::GetActorOfClass(GetWorld(), AMyActor::StaticClass()));

    const bool bUseGolfCart = GolfCart != nullptr;
    const FString Text = bUseGolfCart ? GolfCart->DisplayText
                                      : (Actor ? Actor->DisplayText : TEXT("Aguardando GPS fix..."));

    // ── Painel de dados — canto superior esquerdo ────────────────────────────
    ...
    const float BoxH  = PadY * 2.f + (Lines.Num() + 1) * LineH;
    ...
    // Título
    DrawText(TEXT("[ Gemeo Digital — EKF ]"),
             FLinearColor(0.3f, 0.8f, 1.f, 1.f),
             BoxX + PadX, BoxY + PadY, nullptr, 1.0f);

    // Linhas de dados
    for (int32 i = 0; i < Lines.Num(); ++i)
    {
        DrawText(Lines[i], FLinearColor::White,
                 BoxX + PadX, BoxY + PadY + LineH * (i + 1), nullptr, 1.0f);
    }
}
